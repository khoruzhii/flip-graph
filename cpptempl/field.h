...
struct B2 {
    ...
    static B2 unit(int position) {
        return B2(1ULL << position);
    }
    ...
    bool operator<(const B2& o) const {
        return val < o.val;
    }
};
...
struct B3 {
    ...
    static B3 unit(int position) {
        return B3(1ULL << position, 0);  // коэффициент 1 в позиции position
    }
    ...
    bool operator<(const B3& o) const {
...
namespace std {
    template<>
    struct hash<B2> {
        size_t operator()(const B2& b) const noexcept {
    ...
    template<>
    struct hash<B3> {
        size_t operator()(const B3& b) const noexcept {
            // Combine hashes of lo and hi
