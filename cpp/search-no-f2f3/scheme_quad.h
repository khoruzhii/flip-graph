...
private:
    ...
    // Compare packed values; swap if u > v.
    inline static bool needs_swap_uv(const Field& u, const Field& v) {
        return pack_field(u) > pack_field(v);
    }

    // Swap u and v inside term t if needed; uses 2x del/add at most.
    void canonicalize_term(int t) {
        Field u = data[3*t + 0];
        Field v = data[3*t + 1];
        if (!needs_swap_uv(u, v)) return;
...
public:
    ...
    void add_term(const Field& u, const Field& v, const Field& w) {
        ...
        canonicalize_term(term_idx);
    }
    
    // Set component value with canonical u<=v by packed order.
    void set(int term_idx, int comp, const Field& new_val) {
        ...
        // Drop entire term if any component becomes zero (kept as before)
        if (new_val.is_zero()) {
            for (int j = 0; j < 3; ++j) {
        ...
        // w-component: no special handling, as before
        if (comp == 2) {
            del(term_idx, 2, old_val);
            data[idx] = new_val;
            add(term_idx, 2, new_val);
            return;
        }

        // u or v is being changed: enforce canonical order by packed(u) <= packed(v)
        Field u_old = data[3*term_idx + 0];
        Field v_old = data[3*term_idx + 1];

        Field u_new = (comp == 0) ? new_val : u_old;
        Field v_new = (comp == 1) ? new_val : v_old;

        if (!needs_swap_uv(u_new, v_new)) {
            // Single-component update: minimal 1x del/add
            del(term_idx, comp, old_val);
            data[idx] = new_val;
            add(term_idx, comp, new_val);
        } else {
            // Swap needed: minimal 2x del/add on u and v
            del(term_idx, 0, u_old);
            del(term_idx, 1, v_old);
            data[3*term_idx + 0] = v_new;
            data[3*term_idx + 1] = u_new;
            add(term_idx, 0, v_new);
            add(term_idx, 1, u_new);
        }
    }
    ...
    bool plus() {
        ...
        // Read current terms
        Field a1 = data[3*idx1 + 0];
        Field b1 = data[3*idx1 + 1];
        Field c1 = data[3*idx1 + 2];

        Field a2 = data[3*idx2 + 0];
        Field b2 = data[3*idx2 + 1];
        Field c2 = data[3*idx2 + 2];

        // Avoid identical component pairs as before
        if (a1 == a2 || b1 == b2 || c1 == c2) {
            return plus(); // retry
        }

        // New values (same algebra as before)
        Field new_b1 = b1 - b2;
        Field new_a2 = a1;
        Field new_c2 = c1 - c2;
        Field new_a3 = a1 + a2;
        ...
        // Apply via set() so u,v are canonicalized with minimal ops
        set(idx1, 1, new_b1);   // update v of term idx1
        set(idx2, 0, new_a2);   // update u of term idx2
        set(idx2, 2, new_c2);   // update w of term idx2

        // Add third term; add_term will canonicalize it (see below)
